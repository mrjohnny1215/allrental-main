...
# 4종만 추출
target_out={c:[] for c in TARGET}
allcats={}
for u,r in out.items():
    allcats.setdefault(r['category'],0)
    if r['category'] in TARGET:
        rec={
            'url':u,'title':r.get('title',''),'category':r['category'],
            'it_price':r.get('it_price',''),'rental_periods':r.get('rental_periods',[]),
            'maintenance_cycles':r.get('maintenance_cycles',[]),'colors':r.get('colors',[]),
            'sizes':r.get('sizes',[]),'care_types':r.get('care_types',[]),
            'detail_images':r.get('detail_images',[]),'period_prices':r.get('period_prices',{}),
            'not_available':r.get('not_available',False),
        }
        target_out[r['category']].append(rec)

# 전체 단일 JSON
combined={'total':len(out),'categories':allcats,'target_categories':TARGET,
          'target_products':target_out,'all_products':out}
...
print('전체 상품:',len(out))
print('카테고리 분포:', {k:v for k,v in sorted(allcats.items(), key=lambda x:-x[1])})
for c in TARGET:
    print(f'  {TARGET[c]}({c}): {len(target_out[c])}개')
