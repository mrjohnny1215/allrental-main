...
def crawl_product_detail(session, url):
    ...
    try:
        ...
        # 5. 색상/사이즈 추출 (매트리스는 사이즈 정보, 여러 셀렉트 id/class 시도)
        color_found = False
        candidate_ids = [
            ...
            if sel:
                for option in sel.find_all('option'):
                    ...
                    if value and text and text != '선택':
                        ...
                        # 매트리스: rental_option_3 는 사이즈/관리등급
                        if cid == '#rental_option_3' and color_name not in product_data['sizes']:
                            product_data['sizes'].append(color_name)
                        elif color_name and color_name not in product_data['colors']:
        ...
        if not color_found:
            for label in soup.select('label'):
                ...
                if '색상' in lbl_text or '컬러' in lbl_text or 'color' in lbl_text.lower() or '사이즈' in lbl_text:
