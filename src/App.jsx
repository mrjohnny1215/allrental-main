      ...
      <header className="bg-gradient-to-r from-blue-700 via-blue-600 to-blue-700 text-white shadow-xl sticky top-0 z-40">
        ...
        <div className="bg-blue-900/40 border-t border-white/10">
          <div className="max-w-7xl mx-auto px-4">
            <div className="flex overflow-x-auto gap-1 py-0.5">
              {SITE_CONFIG.categories.map((c) => (
                <button key={c.key} onClick={() => { setActiveCategory(c.key); setBrandFilter('all'); setSearch(''); setFuncFilter('all'); setTypeFilter('all'); setMethodFilter('all'); setPriceFilter('all'); setAreaFilter('all'); setAirFuncFilter('all'); setMattressTypeFilter('all'); }}
                  className={`flex flex-col items-center gap-px px-2 py-0.5 rounded-lg transition-all flex-shrink-0 w-16 ${activeCategory === c.key ? 'bg-white text-blue-700 shadow-md font-black' : 'bg-white/15 text-white hover:bg-white/30 font-semibold'}`}>
                  <div className={`w-8 h-8 rounded-md overflow-hidden border-2 flex items-center justify-center ${activeCategory === c.key ? 'bg-white border-white' : 'bg-white/90 border-transparent'}`}>
                  ...
                  <span className={`text-[9px] font-bold whitespace-nowrap ${activeCategory === c.key ? 'text-blue-700' : 'text-blue-100'}`}>{c.name}</span>
                </button>
              ))}
            </div>
          </div>
