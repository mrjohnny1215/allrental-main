            ...
            <div className="flex overflow-x-auto gap-1.5 py-0.5">
              {SITE_CONFIG.categories.map((c) => (
                <button key={c.key} onClick={() => { setActiveCategory(c.key); setBrandFilter('all'); setSearch(''); setFuncFilter('all'); setTypeFilter('all'); setMethodFilter('all'); setPriceFilter('all'); setAreaFilter('all'); setAirFuncFilter('all'); setMattressTypeFilter('all'); }}
                  className={`flex flex-col items-center gap-px px-2 py-0.5 rounded-lg transition-all flex-shrink-0 w-20 ${activeCategory === c.key ? 'bg-white text-blue-700 shadow-md font-black' : 'bg-white/15 text-white hover:bg-white/30 font-semibold'}`}>
